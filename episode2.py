...
#                             'Leadership': ep_bias_list[6], 'Composure': ep_bias_list[7], 'Teamwork': ep_bias_list[8], 'Theatrics': ep_bias_list[9],
#                         'Devotion': ep_bias_list[10], 'Political Drive': ep_bias_list[11], 'Humor': ep_bias_list[12]}

episode_types = episode_type.return_ep_types(1)

class Episode:
    def __init__(self, week_number: int, contestants: list[Contestant], rng_seed: int, episode_type = None):
        self.__week_number = week_number
        self.__contestants = contestants
        self.__contestant_number = len(contestants)
        self.__rng_seed = rng_seed + random.Random(rng_seed).randint(1,2500)
        self.__high_count = 2   #Default
        self.__btm_count = 2    #Default
        self.__low_count = 1
        self.__ep_type = episode_type
        ...
        seed_increment = random.Random(self.__rng_seed).randint(1, 999)
        ...
        if episode_type is None:
            ...
            random.Random(self.__rng_seed).shuffle(contestants)
            ...
            for contestant in contestants:
                ...
                rng_score = random.Random(self.__rng_seed + seed_increment).randint(1, 100)
                seed_increment = seed_increment + seed_increment
                random_share = rng_score * random_bias
                score_share = (contestant.score_average) * score_bias
                if self.__week_number <= 4:
                    half1 = random.Random(self.__rng_seed + seed_increment).randint(-25, 25) * 0.5
                    seed_increment = seed_increment + seed_increment
                ...
                seed_increment = seed_increment + seed_increment
                chance_of_showstopper = ((random.Random(self.__rng_seed + seed_increment).randint(1, 100) * 0.75) + (
                            contestant.star_power * 0.15) + (contestant.score_average * 0.1))
                ...
                seed_increment = seed_increment + seed_increment
                losing_steam_chance = random.Random(self.__rng_seed + seed_increment).randint(1, 100)
                seed_increment = seed_increment + seed_increment
                ...
                seed_increment = seed_increment + seed_increment
                ceiling_max = contestant.score_average + random.Random(self.__rng_seed + seed_increment).randint(1, 20)
                if ceiling_max > 100:
                    ceiling_max = 100
                if ceiling_max < 60:
                    ceiling_max = 60
                seed_increment = seed_increment + seed_increment
                ceiling_rng = random.Random(self.__rng_seed + seed_increment).randint(60, round(ceiling_max))

                seed_increment = seed_increment + seed_increment
                great_contestant_score_floor = contestant.score_average - (contestant.score_average * (
                            random.Random(self.__rng_seed + seed_increment).randint(30, 50) / 100))
                seed_increment = seed_increment + seed_increment

                if contestant.wins >= 2 and (
                        contestant.wins + contestant.highs) > 6 and self.__week_number >= 7 and losing_steam_chance <= 86 and contestant.ceiling_steam_change == False:
                ...
                seed_increment = seed_increment + seed_increment
                temp_ceiling_rng = random.Random(self.__rng_seed + seed_increment).randint(1, 100)
                ...
                if contestant.wins == 0 and contestant.btms >= 1 and (
                        (contestant.safes + contestant.lows >= 3) or contestant.safes >= 4) and temp_ceiling_rng < 85:
                ...
                seed_increment = seed_increment + seed_increment
                final_fumble = random.Random(self.__rng_seed + seed_increment).randint(1, 100)
                seed_increment = seed_increment + seed_increment
                ...
                seed_increment = seed_increment + seed_increment
                contestant_score = (contestant_score * (1 - rngize_percent)) + (rngize_rating * rngize_percent)
                score_unrngize = contestant.score_average
                if (contestant.wins + contestant.highs) >= 5:
                    score_unrngize = score_unrngize + 10
                    if score_unrngize > 100:
                        score_unrngize = 100
                if self.__week_number > 6:
                    contestant_score = (contestant_score * 0.85) + (score_unrngize * 0.15)
            ...
            contestant_stat_names = ['Strength', 'Speed', 'Luck',
                            'Fashion', 'Love', 'Beauty',
                            'Leadership', 'Composure',
                            'Teamwork', 'Theatrics',
                            'Devotion', 'Political Drive',
                            'Humor', 'Craftsmanship',
                            'Intelligence']
            for type in episode_types:
                if type.ep_type == self.__ep_type.ep_type:
                    for contestant in contestants:
                        contestant_weekly_score = []
                        preliminary_score = 0
                        for stat_name in contestant_stat_names:
                            preliminary_score = preliminary_score + (self.__ep_type.ep_biases[stat_name] * contestant.stats[stat_name])



                        #RNG not multiplicative
                        seed_increment += 1
                        randomization_additive = 25
                        # if self.__ep_type.ep_type[0:6] == "random":
                        #     randomization_additive = 80

                        randomization_additive_apply = random.Random(self.__rng_seed+seed_increment).randint(1,100)
                        seed_increment += 1
                        random_ratio = (randomization_additive / 100) * randomization_additive_apply
                        score_ratio = ((100 - randomization_additive) / 100) * preliminary_score

                        preliminary_score = random_ratio  + score_ratio

                        # randomize_range = 60
                        # randomize_additive = random.Random(self.__rng_seed+seed_increment).randint(-abs(randomize_range),randomize_range)
                        # preliminary_score = preliminary_score + randomize_additive
                        # seed_increment = seed_increment + 1

                        if preliminary_score < contestant.score_floor:
                            preliminary_score = contestant.score_floor
                            preliminary_score = preliminary_score + random.Random(self.__rng_seed+seed_increment).uniform(-2.33,2.33)
                            seed_increment += 1

                        if contestant.score_floor > 45:
                            preliminary_score = preliminary_score + 10

                        if preliminary_score <= 50:
                            preliminary_score = preliminary_score + contestant.rigor_morris
                            if preliminary_score > 79:
                                preliminary_score = 79



                        # RNG
                        randomization_percent = 6
                        randomization_apply = (random.Random(self.__rng_seed + seed_increment).randint(
                            -abs(randomization_percent), randomization_percent) / 100) + 1
                        seed_increment += 1
                        preliminary_score = preliminary_score * randomization_apply
                        ...
                        contestant_score = round(preliminary_score,2)
                        contestant.ep_count = contestant.ep_count + 1
                        contestant.score = contestant.score + contestant_score
                        contestant_weekly_score.append(contestant)
                        contestant_weekly_score.append(contestant_score)
        ...
        if self.__contestant_number >= 4:
            seed_increment = seed_increment + seed_increment
            good_contestant_rigor_morris_rng = random.Random(self.__rng_seed + seed_increment).randint(1, 100)
            good_contestant_rigor_morris_chance = 90
            seed_increment = seed_increment + seed_increment

            if self.__contestant_weekly_scores[self.__contestant_number-1][0].wins >=3 and good_contestant_rigor_morris_rng < good_contestant_rigor_morris_chance and self.__contestant_weekly_scores[self.__contestant_number-2][0].wins <= 2:
            ...
            if self.__contestant_weekly_scores[self.__contestant_number-1][0].wins >=2 and good_contestant_rigor_morris_rng < good_contestant_rigor_morris_chance and self.__contestant_weekly_scores[self.__contestant_number-2][0].wins <= 1:
                self.__contestant_weekly_scores[self.__contestant_number - 1][0], self.__contestant_weekly_scores[self.__contestant_number-2][0] = self.__contestant_weekly_scores[self.__contestant_number-2][0], self.__contestant_weekly_scores[self.__contestant_number - 1][0]

            seed_increment = seed_increment + seed_increment
            too_many_wins_rigor_morris = random.Random(self.__rng_seed + seed_increment).randint(1, 100)
            seed_increment = seed_increment + seed_increment
        ...
        seed_increment = seed_increment + seed_increment
        winner_morris2 = random.Random(self.__rng_seed + seed_increment).randint(1, 100)
        seed_increment = seed_increment + seed_increment

        if (self.__contestant_weekly_scores[self.__contestant_number-1][0].btms < self.__contestant_weekly_scores[self.__contestant_number-2][0].btms) and self.__contestant_weekly_scores[self.__contestant_number-2][0].btms >= 3 and 90 > winner_morris2:
            self.__contestant_weekly_scores[self.__contestant_number - 1][0], \
            self.__contestant_weekly_scores[self.__contestant_number - 2][0] = \
            self.__contestant_weekly_scores[self.__contestant_number - 2][0], \
            self.__contestant_weekly_scores[self.__contestant_number - 1][0]

        if (self.__contestant_weekly_scores[self.__contestant_number-1][0].wins > self.__contestant_weekly_scores[self.__contestant_number-2][0].wins or self.__contestant_weekly_scores[self.__contestant_number-1][0].score_average - self.__contestant_weekly_scores[self.__contestant_number-2][0].score_average > 18) and 80 > winner_morris2:
            self.__contestant_weekly_scores[self.__contestant_number - 1][0], \
            self.__contestant_weekly_scores[self.__contestant_number - 2][0] = \
            self.__contestant_weekly_scores[self.__contestant_number - 2][0], \
            self.__contestant_weekly_scores[self.__contestant_number - 1][0]
        ...
        for contestant in contestants:
            ...
            if week_number == 1:
                if contestant.contestant_name == self.__winner:
                    contestant.score = contestant.score + 6
            ...
            if contestant.safes >= 3 and week_number <= 7 and contestant.overly_safe_flag == False:
                ...
                seed_increment = seed_increment + seed_increment
                safe_change = random.Random(self.__rng_seed + seed_increment).randint(-30, 15)
